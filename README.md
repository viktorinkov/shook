...
<table>
<tr>
<th width="50%">The simple-english plugin alone. Its skill did not fire.</th>
<th width="50%">Both plugins. Mode <code>on</code>.</th>
</tr>
<tr>
<td valign="top">
...
Claude rewrites the reply once per turn. The status line badge shows the last lint score, for example `[STE:STRICT 0.3]`.
...
Start a new session. Type `/ste on` for the rules at session start and a reminder on every prompt. Type `/ste strict` for the reminder plus the lint gate. Type `/ste off` to stop. Type `/ste project strict` to limit the mode to one repo.

Requirements: Claude Code, the simple-english plugin, `jq`, and `python3` (strict mode only). No Node. No telemetry. Six shell files.

The same hooks run in Codex CLI and Antigravity CLI (`agy`), with strict mode. In Copilot CLI, the rules load at session start only. See [`docs/other-harnesses.md`](docs/other-harnesses.md).

## Numbers

50 prompts, one run per arm, `claude-sonnet-5`. Scored with the simple-english plugin's own `ste_lint.py`. Full table, method, and every raw reply: [`evals/results/RESULTS.md`](evals/results/RESULTS.md).
...
How to read it: the simple-english skill fired on its own in 24 of 50 replies. The simple-english output style is a good one-shot. This plugin adds a reminder next to every prompt, and strict mode adds the lint gate. Strict mode blocked 6 of 50 replies and used about 550 more output tokens per reply than `hook-on`.
...
Reproduce: `python3 evals/bench.py`. The script needs a logged-in Claude Code CLI and the installed simple-english plugin.
...
**Why not just prompt it?** A prompt line is one instruction among many. In the benchmark, the simple-english skill fired on its own in 24 of 50 replies. This plugin does not depend on a decision. It runs every turn.

**Why not the simple-english output style?** The output style is a good one-shot. It has no reminder per turn and no gate. See the `style` row in the table.
